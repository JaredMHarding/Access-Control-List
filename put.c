...
int main(int argc, char** argv) {
    if (argc != 3) {
        fprintf(stderr,"Usage: ./put source destination\n");
        exit(1);
    }
    ...
    uid_t uid = getuid(); // user running the put program
    uid_t euid = geteuid(); // user who owns or will own the dest file
    char src[MAXPATHSIZE]; // uid's file to be copied
    char dest[MAXPATHSIZE]; // euid's location to place the copy
    strncpy(src,argv[1],MAXPATHSIZE);
    strncpy(dest,argv[2],MAXPATHSIZE);
    // make the string for the <dest>.access file
    char acl[MAXPATHSIZE+7];
    strncpy(acl,dest,MAXPATHSIZE);
    strncat(acl,".access",7);
    // get stat info for the <dest>.access file
    ...
    while (cursor != NULL) {
        if (strncmp(realpw->pw_name,cursor->name,MAXNAMESIZE) == 0) {
            if ((cursor->access == 'w') || (cursor->access == 'b')) {
                // user is authorized
                break;
            }
            cursor = NULL;
    ...
    if (cursor == NULL) {
        fprintf(stderr,"User is not authorized to write to destination file '%s'\n",dest);
        exit(1);
    }
    freeNAP(aclnap);
    ...
    if (seteuid(uid) == -1) {// open src as the real user
        perror("seteuid(uid)");
        exit(1);
    }
    ...
    if (seteuid(euid) == -1) {// set euid back to normal
        perror("seteuid(euid)");
        exit(1);
    }
